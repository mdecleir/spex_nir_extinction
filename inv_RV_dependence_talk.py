...
def plot_inv_rv_dep(
    ...
    """
    Plot the relationship between A(lambda)/A(V) and 1/R(V)-1/3.1 at wavelengths "plot_waves"
    ...
    inv_RVs : np.ndarray
        Numpy array with 1/R(V) values and uncertainties
    ...
    Returns
    -------
    Plot of A(lambda)/A(V) vs. 1/R(V)-1/3.1 at wavelengths "plot_waves"
    """
    fig, ax = plt.subplots(
...
def table_inv_rv_dep(
    outpath, table_waves, fit_slopes, fit_intercepts, fit_stds, norm="V"
):
    """
...
def plot_inv_RV_lit(outpath, fit_slopes, fit_intercepts, fit_stds):
    ...
    # ax.set_ylabel(r"$A(\lambda)/A(V)$", fontsize=1.2 * fs)
...
def fit_plot_inv_rv_dep(
    ...
    for j, wave in enumerate(waves):
        ...
        std = np.sqrt(
            np.sum((fitted_line(inv_RV_vals[mask] - 1 / 3.1) - alavs[j][mask]) ** 2)
            / (npts - 2)
        )
    ...
    ax[0].plot(
        waves[short_waves],
        fit_intercepts(waves[short_waves]),
        color="k",
        ls="--",
        # alpha=0.9,
    ...
    plt.subplots_adjust(hspace=0)
    plt.savefig(
        plot_path + "inv_RV_slope_inter" + str(norm) + ".pdf", bbox_inches="tight"
    )
    ...
    plt.legend()
    plt.savefig(plot_path + "inv_inter_res" + str(norm) + ".pdf", bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    fit_plot_inv_rv_dep(inpath, plot_path, table_path, good_diffuse, good_dense)

    # fit and plot the RV dependence when normalizing to 1 micron instead of to the V-band
    # fit_plot_inv_rv_dep(inpath, plot_path, table_path, good_diffuse, good_dense, norm=1)
